...
struct SettingsView: View {
    ...
    struct DraftSettings {
        var theme: Int = 0
        var lineSpace: Double = 8.0
        var scrollMode: Int = 0
        var linesPerScroll: Int = 1
        var hideSharing: Bool = false
        var opacity: Double = 1.0
        var mirror: Bool = false
        var fontFamily: Int = 0
        var alignment: Int = 0
        var highContrast: Bool = false
        var editablePresets: [(name: String, speed: Double)] = []
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                settingsSection("Display") {
                    ...
                    settingsRow("Opacity", value: "\(Int(draft.opacity * 100))%")
                    Slider(value: $draft.opacity, in: 0.3...1.0, step: 0.05)
                    
                    Toggle("Mirror text", isOn: $draft.mirror).font(.system(size: 12))
                ...
                settingsSection("Typography") {
                    ...
                    settingsRow("Line Spacing", value: "\(Int(draft.lineSpace))")
                ...
                settingsSection("Controls") {
                    ...
                    Picker("", selection: $draft.linesPerScroll) {
                        Text("1 line").tag(1)
                        Text("3 lines").tag(3)
                        Text("5 lines").tag(5)
                    }
                        ...
                        HStack(spacing: 6) {
                            Button(action: {
                                NotificationCenter.default.post(
                                    ...
                                    object: draft.editablePresets[i].speed
                                )
                                SettingsWindowManager.sharedWindow?.close()
                            }) {
                                HStack {
                                    Text(draft.editablePresets[i].name).font(.system(size: 12))
                                    Spacer()
                                    Text("\(String(format: "%.1f", draft.editablePresets[i].speed))×")
                                        .font(.system(size: 11, design: .monospaced))
                                        .foregroundColor(.secondary)
                                }
                            }
                            .buttonStyle(.plain)
                            Button(action: { draft.editablePresets.remove(at: i) }) {
                ...
                HStack {
                    ...
                    Button("Save") {
                        themePreference = draft.theme
                        lineSpace = draft.lineSpace
                        scrollMode = draft.scrollMode
                        linesPerScroll = draft.linesPerScroll
                        hideFromScreenSharing = draft.hideSharing
                        windowOpacity = draft.opacity
                        mirrorMode = draft.mirror
        ...
        .onAppear {
            draft = DraftSettings(
                ...
                hideSharing: hideFromScreenSharing,
    ...
    func addPreset() {
        ...
        draft.editablePresets.append((name: newPresetName, speed: spd))
        newPresetName = ""
        newPresetSpeed = ""
    }
